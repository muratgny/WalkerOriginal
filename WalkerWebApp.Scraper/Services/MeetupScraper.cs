...
namespace WalkerWebApp.Scraper.Services
{
    public class MeetupScraper
    {
        ...
        public void GetListOfCityAndState()
        {
            ...
            using (var context = new ScraperDBContext())
            {
               var cities = context.Cities.OrderBy(x => x.Id).Skip(currentBatch++ * batchSize).Take(batchSize).ToList();
                ...
                {
                    IterateOverRunningClubs(city.CountryCode.ToLower(), city.CityName.ToLower());
                    if(city.Id == 40000)
                    {
                       done = true;
                    }
        ...
        public void IterateOverRunningClubs(string country, string city)
        {
            try
            {
                _driver.Navigate().GoToUrl($"https://www.meetup.com/find/?suggested=true&source=GROUPS&keywords=running%20club&location=us--{country}--{city}");
